!function(){
	//css2 part
    var css2 = [
        //font
        'font:',
        'color:',
        'color:#000;',
        'color:rgb(0,0,0);',
        'font-family:',
        'font-size:',
        'font-size-adjust/*deprecated*/:none;',
        'font-stretch/*deprecated*/:normal;',
        'font-style:normal;',
        'font-style:italic;',
        'font-style:oblique;',
        'font-weight:normal;',
        'font-weight:bold;',
        'font-weight:bolder;',
        'font-weight:lighter;',
        'font-weight:100;',
        'font-weight:200;',
        'font-weight:300;',
        'font-weight:400;',
        'font-weight:500;',
        'font-weight:600;',
        'font-weight:700;',
        'font-weight:800;',
        'font-weight:900;',
        'text-decoration:none;',
        'text-decoration:underline;',
        'text-decoration:blink;',
        'text-decoration:overline;',
        'text-decoration:line-through;',
        'text-underline-position:below;',
        'text-underline-position:above;',
        'text-shadow/*deprecated*/:[color length length opacity];',
        'font-variant:normal;',
        'font-variant:small-caps;',
        'text-transform:none;',
        'text-transform:capitalize;',
        'text-transform:uppercase;',
        'text-transform:lowercase;',
        'line-height:',
        'line-height:normal;',
        'letter-spacing:',
        'letter-spacing:normal;',
        'word-spacing:',
        'word-spacing:normal;',
        //text
        'text-indent:',
        'text-overflow:clip;',
        'text-overflow:ellipsis;',
        'vertical-align:auto;',
        'vertical-align:baseline;',
        'vertical-align:sub;',
        'vertical-align:super;',
        'vertical-align:top;',
        'vertical-align:text-top;',
        'vertical-align:middle;',
        'vertical-align:bottom;',
        'vertical-align:text-bottom;',
        'vertical-align:[length];',
        'text-align:left;',
        'text-align:right;',
        'text-align:center;',
        'text-align:justify;',
        'layout-flow:horizontal;',
        'layout-flow:vertical-ideographic;',
        'writing-mode:lr-tb;',
        'writing-mode:tb-rl;',
        'direction:ltr;',
        'direction:rtl;',
        'direction:inherit;',
        'unicode-bidi:normal;',
        'unicode-bidi:bidi-override;',
        'unicode-bidi:embed;',
        'word-break:normal;',
        'word-break:break-all;',
        'word-break:keep-all;',
        'line-break:normal;',
        'line-break:strict;',
        'white-space:normal;',
        'white-space:pre;',
        'white-space:nowrap;',
        'word-wrap:normal;',
        'word-wrap:break-word;',
        'word-wrap:break-word;',
        'text-autospace:none;',
        'text-autospace:ideograph-alpha;',
        'text-autospace:ideograph-numeric;',
        'text-autospace:ideograph-parenthesis;',
        'text-autospace:ideograph-space;',
        'text-kashida-space:[length];',
        'text-justify:auto;',
        'text-justify:distribute;',
        'text-justify:distribute-all-lines;',
        'text-justify:distribute-center-last;',
        'text-justify:inter-cluster;',
        'text-justify:inter-ideograph;',
        'text-justify:inter-word;',
        'text-justify:kashida;',
        'text-justify:newspaper;',
        'text-justify:auto;',
        'ruby-align:auto;',
        'ruby-align:left;',
        'ruby-align:center;',
        'ruby-align:right;',
        'ruby-align:distribute-letter;',
        'ruby-align:distribute-space;',
        'ruby-align:line-edge;',
        'ruby-overhang:auto;',
        'ruby-overhang:whitespace;',
        'ruby-overhang:none;',
        'ruby-position:above;',
        'ruby-position:inline;',
        'ime-mode:auto;',
        'ime-mode:active;',
        'ime-mode:inactive;',
        'ime-mode:disabled;',
        'layout-grid:[layout-grid-mode layout-grid-type layout-grid-line layout-grid-char];',
        'layout-grid-char:none;',
        'layout-grid-char:auto;',
        'layout-grid-char:[length];',
        'layout-grid-line:none;',
        'layout-grid-line:auto;',
        'layout-grid-line:[length];',
        'layout-grid-mode:both;',
        'layout-grid-mode:line;',
        'layout-grid-mode:char;',
        'layout-grid-mode:none;',
        'layout-grid-type:loose;',
        'layout-grid-type:strict;',
        'layout-grid-type:fixed;',
        //background
        'background:',
        'background:transparent none repeat scroll 0% 0%;',
        'background-attachment:scroll;',
        'background-attachment:fixed;',
        'background-color:',
        'background-color:transparent;',
        'background-color:#000;',
        'background-color:rgb(0,0,0);',
        'background-image:none;',
        'background-image:url(images/);',
        'background-position:',
        'background-position-x:',
        'background-position-y:',
        'background-repeat:repeat;',
        'background-repeat:no-repeat;',
        'background-repeat:repeat-x;',
        'background-repeat:repeat-y;',
        'layer-background-color:',
        'layer-background-color:transparent;',
        'layer-background-image:none;',
        'layer-background-image:url();',
        //positioning
        'position:static;',
        'position:absolute;',
        'position:fixed;',
        'position:relative;',
        'z-index:',
        'top:',
        'right:',
        'bottom:',
        'left:',
        //dimensions
        'height:',
        'height:auto;',
        'max-height:',
        'max-height:none;',
        'min-height:',
        'min-height:none;',
        'width:',
        'width:auto;',
        'max-width:',
        'max-width:none;',
        'min-width:',
        'min-width:none;',
        //layout
        'clear:none;',
        'clear:left;',
        'clear:right;',
        'clear:both;',
        'float:none;',
        'float:left;',
        'float:right;',
        'clip:auto;',
        'clip:rect(0 0 0 0);',
        'overflow:visible;',
        'overflow:auto;',
        'overflow:hidden;',
        'overflow:scroll;',
        'overflow-x:visible;',
        'overflow-x:auto;',
        'overflow-x:hidden;',
        'overflow-x:scroll;',
        'overflow-y:visible;',
        'overflow-y:auto;',
        'overflow-y:hidden;',
        'overflow-y:scroll;',
        'display:block;',
        'display:none;',
        'display:inline;',
        'display:compact;',
        'display:marker;',
        'display:inline-table;',
        'display:list-item;',
        'display:run-in;',
        'display:table;',
        'display:table-caption;',
        'display:table-cell;',
        'display:table-column;',
        'display:table-column-group;',
        'display:table-footer-group;',
        'display:table-header-group;',
        'display:table-row;',
        'display:table-row-group;',
        'visibility:inherit;',
        'visibility:visible;',
        'visibility:collapse;',
        'visibility:hidden;',
        //margins
        'margin:',
        'margin-top:',
        'margin-right:',
        'margin-bottom:',
        'margin-left:',
        //outlines
        'outline:',
        'outline-color:',
        'outline-color:invert;',
        'outline-style:none;',
        'outline-style:dotted;',
        'outline-style:dashed;',
        'outline-style:solid;',
        'outline-style:double;',
        'outline-style:groove;',
        'outline-style:ridge;',
        'outline-style:inset;',
        'outline-style:outset;',
        'outline-width:',
        'outline-width:medium;',
        'outline-width:thin;',
        'outline-width:thick;',
        //borders
        'border:',
        'border-color:',
        'border-style:none;',
        'border-style:hidden;',
        'border-style:dotted;',
        'border-style:dashed;',
        'border-style:solid;',
        'border-style:double;',
        'border-style:groove;',
        'border-style:ridge;',
        'border-style:inset;',
        'border-style:outset;',
        'border-width:',
        'border-width:medium;',
        'border-width:thin;',
        'border-width:thick;',
        'border-top:',
        'border-top-color:',
        'border-top-style:none;',
        'border-top-style:hidden;',
        'border-top-style:dotted;',
        'border-top-style:dashed;',
        'border-top-style:solid;',
        'border-top-style:double;',
        'border-top-style:groove;',
        'border-top-style:ridge;',
        'border-top-style:inset;',
        'border-top-style:outset;',
        'border-top-width:',
        'border-top-width:medium;',
        'border-top-width:thin;',
        'border-top-width:thick;',
        'border-right:',
        'border-right-color:',
        'border-right-style:none;',
        'border-right-style:hidden;',
        'border-right-style:dotted;',
        'border-right-style:dashed;',
        'border-right-style:solid;',
        'border-right-style:double;',
        'border-right-style:groove;',
        'border-right-style:ridge;',
        'border-right-style:inset;',
        'border-right-style:outset;',
        'border-right-width:',
        'border-right-width:medium;',
        'border-right-width:thin;',
        'border-right-width:thick;',
        'border-bottom:',
        'border-bottom-color:',
        'border-bottom-style:none;',
        'border-bottom-style:hidden;',
        'border-bottom-style:dotted;',
        'border-bottom-style:dashed;',
        'border-bottom-style:solid;',
        'border-bottom-style:double;',
        'border-bottom-style:groove;',
        'border-bottom-style:ridge;',
        'border-bottom-style:inset;',
        'border-bottom-style:outset;',
        'border-bottom-width:',
        'border-bottom-width:medium;',
        'border-bottom-width:thin;',
        'border-bottom-width:thick;',
        'border-left:',
        'border-left-color:',
        'border-left-style:none;',
        'border-left-style:hidden;',
        'border-left-style:dotted;',
        'border-left-style:dashed;',
        'border-left-style:solid;',
        'border-left-style:double;',
        'border-left-style:groove;',
        'border-left-style:ridge;',
        'border-left-style:inset;',
        'border-left-style:outset;',
        'border-left-width:',
        'border-left-width:medium;',
        'border-left-width:thin;',
        'border-left-width:thick;',
        //generated content
        'include-source:url();',
        'quotes:',
        'quotes:none;',
        'content:attr(alt);',
        'content:counter(name);',
        'content:counter(name, list-style-type);',
        'content:counters(name, string);',
        'content:counters(name, string, list-style-type);',
        'content:no-close-quote;',
        'content:no-open-quote;',
        'content:close-quote;',
        'content:open-quote;',
        'content:[string];',
        'content:url(url);',
        'counter-increment:',
        'counter-increment:none;',
        'counter-reset:',
        'counter-reset:none;',
        //paddings
        'padding:',
        'padding-top:',
        'padding-right:',
        'padding-bottom:',
        'padding-left:',
        //lists
        'list-style:',
        'list-style-image:none;',
        'list-style-image:url(images/);',
        'list-style-position:outside;',
        'list-style-position:inside;',
        'list-style-type:disc;',
        'list-style-type:circle;',
        'list-style-type:square;',
        'list-style-type:decimal;',
        'list-style-type:lower-roman;',
        'list-style-type:upper-roman;',
        'list-style-type:lower-alpha;',
        'list-style-type:upper-alpha;',
        'list-style-type:none;',
        'list-style-type:armenian;',
        'list-style-type:cjk-ideographic;',
        'list-style-type:georgian;',
        'list-style-type:lower-greek;',
        'list-style-type:hebrew;',
        'list-style-type:hiragana;',
        'list-style-type:hiragana-iroha;',
        'list-style-type:katakana;',
        'list-style-type:katakana-iroha;',
        'list-style-type:lower-latin;',
        'list-style-type:upper-latin;',
        'marker-offset/*deprecated*/:',
        'marker-offset:auto;',
        //table
        'border-collapse:separate;',
        'border-collapse:collapse;',
        'border-spacing:',
        'caption-side:top;',
        'caption-side:right;',
        'caption-side:bottom;',
        'caption-side:left;',
        'empty-cells:show;',
        'empty-cells:hide;',
        'table-layout:auto;',
        'table-layout:fixed;',
        'speak-header/*deprecated*/:once;',
        'speak-header/*deprecated*/:always;',
        //scrollbar
        'scrollbar-3dlight-color:',
        'scrollbar-highlight-color:',
        'scrollbar-face-color:',
        'scrollbar-arrow-color:',
        'scrollbar-shadow-color:',
        'scrollbar-darkshadow-color:',
        'scrollbar-base-color:',
        'scrollbar-track-color:',
        //printing
        'page/*deprecated*/:',
        'page/*deprecated*/:auto;',
        'page-break-after:auto;',
        'page-break-after:always;',
        'page-break-after:avoid;',
        'page-break-after:left;',
        'page-break-after:right;',
        'page-break-after:null;',
        'page-break-before:auto;',
        'page-break-before:always;',
        'page-break-before:avoid;',
        'page-break-before:left;',
        'page-break-before:right;',
        'page-break-before:null;',
        'page-break-inside:auto;',
        'page-break-inside:avoid;',
        'marks/*deprecated*/:none;',
        'marks/*deprecated*/:crop;',
        'marks/*deprecated*/:cross;',
        'orphans:',
        'size/*deprecated*/:',
        'size/*deprecated*/:auto;',
        'size/*deprecated*/:portrait;',
        'size/*deprecated*/:landscape;',
        'widows:',
        //aural all deprecated, remove from css2.1
        'voice-family/*deprecated*/:',
        'voice-family/*deprecated*/:child;',
        'voice-family/*deprecated*/:female;',
        'voice-family/*deprecated*/:male;',
        'volume/*deprecated*/:',
        'volume/*deprecated*/:silent;',
        'volume/*deprecated*/:x-soft;',
        'volume/*deprecated*/:soft;',
        'volume/*deprecated*/:medium;',
        'volume/*deprecated*/:loud;',
        'volume/*deprecated*/:x-loud;',
        'elevation/*deprecated*/:',
        'elevation/*deprecated*/:below;',
        'elevation/*deprecated*/:level;',
        'elevation/*deprecated*/:above;',
        'elevation/*deprecated*/:lower;',
        'elevation/*deprecated*/:higher;',
        'azimuth/*deprecated*/:',
        'azimuth/*deprecated*/:left-side;',
        'azimuth/*deprecated*/:far-left;',
        'azimuth/*deprecated*/:left;',
        'azimuth/*deprecated*/:center-left;',
        'azimuth/*deprecated*/:center;',
        'azimuth/*deprecated*/:center-right;',
        'azimuth/*deprecated*/:right;',
        'azimuth/*deprecated*/:far-right;',
        'azimuth/*deprecated*/:right-side;',
        'azimuth/*deprecated*/:leftwards;',
        'azimuth/*deprecated*/:rightwards;',
        'stress/*deprecated*/:',
        'richness/*deprecated*/:',
        'speech-rate/*deprecated*/:',
        'speech-rate/*deprecated*/:x-slow;',
        'speech-rate/*deprecated*/:slow;',
        'speech-rate/*deprecated*/:medium;',
        'speech-rate/*deprecated*/:fast;',
        'speech-rate/*deprecated*/:x-fast;',
        'speech-rate/*deprecated*/:slower;',
        'speech-rate/*deprecated*/:faster;',
        'cue/*deprecated*/:',
        'cue-after/*deprecated*/:none;',
        'cue-after/*deprecated*/:url();',
        'cue-before/*deprecated*/:none;',
        'cue-before/*deprecated*/:url();',
        'pause/*deprecated*/:',
        'pause-after/*deprecated*/:',
        'pause-before/*deprecated*/:',
        'pitch/*deprecated*/:',
        'pitch/*deprecated*/:x-low;',
        'pitch/*deprecated*/:low;',
        'pitch/*deprecated*/:medium;',
        'pitch/*deprecated*/:high;',
        'pitch/*deprecated*/:x-high;',
        'pitch-range/*deprecated*/:',
        'play-during/*deprecated*/:url();',
        'play-during/*deprecated*/:mix;',
        'play-during/*deprecated*/:repeat;',
        'play-during/*deprecated*/:auto;',
        'play-during/*deprecated*/:none;',
        'speak/*deprecated*/:normal;',
        'speak/*deprecated*/:none;',
        'speak/*deprecated*/:spell-out;',
        'speak-numeral/*deprecated*/:continuous;',
        'speak-numeral/*deprecated*/:digits;',
        'speak-punctuation/*deprecated*/:none;',
        'speak-punctuation/*deprecated*/:code;',
        //classification
        'cursor:auto;',
        'cursor:all-scroll;',
        'cursor:col-resize;',
        'cursor:crosshair;',
        'cursor:default;',
        'cursor:hand;',
        'cursor:move;',
        'cursor:help;',
        'cursor:no-drop;',
        'cursor:not-allowed;',
        'cursor:pointer;',
        'cursor:progress;',
        'cursor:row-resize;',
        'cursor:text;',
        'cursor:vertical-text;',
        'cursor:wait;',
        'cursor:w-resize;',
        'cursor:s-resize;',
        'cursor:n-resize;',
        'cursor:e-resize;',
        'cursor:ne-resize;',
        'cursor:sw-resize;',
        'cursor:se-resize;',
        'cursor:nw-resize;',
        'cursor:url();',
        'behavior:url();',
        'filter:',
        'zoom:',
        'zoom:normal;',
        //pseudo-classes
        ':link',
        ':hover',
        ':active',
        ':visited',
        ':first-child',
        ':first',
        ':left',
        ':right',
        ':lang',
        ':focus',
        //pseudo-elements
        ':after',
        ':first-letter',
        ':first-line',
        ':before',
        ':after',
        //at-rules
        '@import url();',
        '@charset',
        '@font-face{font-family:name;src:url(url);sRules};',
        '@fontdef',
        '@page',
        '@media',
        //declaration
        '!important;',
        //units
        'px;',
        'em;',
        'ex;',
        'pt;',
        'pc;',
        'in;',
        'mm;',
        'cm;',
        //angle units
        'deg;',
        'grad;',
        'rad;',
        //time units
        's;',
        'ms;',
        //frequency units
        'Hz;',
        'kHz;',
        //filters & transitions
        'filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=,sizingMethod=,src=);'
        /* 'filter:progid:DXImageTransform.Microsoft.Gradient(enabled=bEnabled,startColorStr=iWidth,endColorStr=iWidth);',
        'filter:progid:DXImageTransform.Microsoft.Alpha(enabled=bEnabled,style=iStyle,opacity=iOpacity,finishOpacity=iFinishOpacity,startX=iPercent,startY=iPercent,finishX=iPercent,finishY=iPercent);',
        'filter:progid:DXImageTransform.Microsoft.BasicImage(enabled=bEnabled,grayScale=bGray,mirror=bMirror,opacity=fOpacity,XRay=bXRay)',
        'filter:progid:DXImageTransform.Microsoft.Blur(enabled=bEnabled,makeShadow=bShadow,pixelRadius=flRadius,shadowOpacity=fOpacity)',
        'filter:progid:DXImageTransform.Microsoft.Chroma(enabled=bEnabled,color=sColor)',
        'filter:progid:DXImageTransform.Microsoft.Compositor(function=sFunction)',
        'filter:progid:DXImageTransform.Microsoft.DropShadow(enabled=bEnabled,color=sColor,offX=iOffsetX,offY=iOffsetY,positive=bPositive)',
        'filter:progid:DXImageTransform.Microsoft.Emboss(enabled=bEnabled,Bias=fBias)',
        'filter:progid:DXImageTransform.Microsoft.Engrave(enabled=bEnabled,Bias=fBias)',
        'filter:progid:DXImageTransform.Microsoft.Glow(enabled=bEnabled,color=sColor,strength=iDistance)',
        'filter:progid:DXImageTransform.Microsoft.Light(enabled=bEnabled)',
        'filter:progid:DXImageTransform.Microsoft.MaskFilter(enabled=bEnabled,color=sColor)',
        'filter:progid:DXImageTransform.Microsoft.Matrix(enabled=bEnabled,SizingMethod=sMethod,FilterType=sType,Dx=fDx,Dy=fDy,M11=fM11,M12=fM12,M21=fM21,M22=fM22)',
        'filter:progid:DXImageTransform.Microsoft.MotionBlur(enabled=bEnabled,add=bAddImage,direction=iOffset,strength=iDistance)',
        'filter:progid:DXImageTransform.Microsoft.Shadow(enabled=bEnabled,color=sColor,direction=iOffset,strength=iDistance)',
        'filter:progid:DXImageTransform.Microsoft.Wave(enabled=bEnabled,add=bAddImage,freq=iWaveCount,lightStrength=iPercentage,phase=iPercentage,strength=iDistance)',
        'filter:progid:DXImageTransform.Microsoft.ICMFilter(colorSpace=sPathFilename,intent=sIntent)',
        'filter:Xray(enabled=bEnabled)',
        'filter:Gray(enabled=bEnabled)',
        'filter:Invert(enabled=bEnabled)',
        'filter:FlipH(enabled=bEnabled)',
        'filter:FlipV(enabled=bEnabled)',
        'filter:Redirect:',
        'filter:progid:DXImageTransform.Microsoft.Barn(enabled=bEnabled,duration=fDuration,motion=sMotion,orientation=sOrientation)',
        'filter:progid:DXImageTransform.Microsoft.Blinds(enabled=bEnabled,duration=fDuration,bands=iBands,Direction=sDirection)',
        'filter:progid:DXImageTransform.Microsoft.CheckerBoard ( enabled=bEnabled , duration=fDuration , squaresX=iColumns , squaresY=iColumns , Direction=sDirection )',
        'filter:progid:DXImageTransform.Microsoft.Fade(enabled=bEnabled,duration=fDuration,overlap=fOverlap)',
        'filter:progid:DXImageTransform.Microsoft.GradientWipe(enabled=bEnabled,duration=fDuration,gradientSize=fGradientWidth,motion=sMotion)',
        'filter:progid:DXImageTransform.Microsoft.Inset(enabled=bEnabled,duration=fDuration)',
        'filter:progid:DXImageTransform.Microsoft.Iris(enabled=bEnabled,duration=fDuration,irisStyle=sShape,motion=sMotion)',
        'filter:progid:DXImageTransform.Microsoft.Pixelate(enabled=bEnabled,duration=fDuration,maxSquare=iWidth)',
        'filter:progid:DXImageTransform.Microsoft.RadialWipe(enabled=bEnabled,duration=fDuration,wipeStyle=sStyle)',
        'filter:progid:DXImageTransform.Microsoft.RandomBars(enabled=bEnabled,duration=fDuration,orientation=sOrientation)',
        'filter:progid:DXImageTransform.Microsoft.RandomDissolve(enabled=bEnabled,duration=fDuration)',
        'filter:progid:DXImageTransform.Microsoft.Slide(enabled=bEnabled,duration=fDuration,bands=iBands,slideStyle=sslideStyle)',
        'filter:progid:DXImageTransform.Microsoft.Spiral(enabled=bEnabled,duration=fDuration,gridSizeX=iColumns,gridSizeY=iColumns)',
        'filter:progid:DXImageTransform.Microsoft.Stretch(enabled=bEnabled,duration=fDuration,stretchStyle=sStretchStyle)',
        'filter:progid:DXImageTransform.Microsoft.Strips(enabled=bEnabled,duration=fDuration,motion=sMotion)',
        'filter:progid:DXImageTransform.Microsoft.Wheel ( enabled=bEnabled , duration=fDuration , spokes=iSpokes )',
        'filter:progid:DXImageTransform.Microsoft.Zigzag(enabled=bEnabled,duration=fDuration,gridSizeX=iColumns,gridSizeY=iColumns)',
        'filter:progid:DXImageTransform.Microsoft.RevealTrans(enabled=bEnabled,duration=fDuration,transition=iTransitionType)',
        'filter:BlendTrans(enabled=bEnabled,duration=fDuration)', */
    ],
    css3 = [
        //borders
        'border-color:',
        'border-color:none;',
        'border-image:',
        'border-image:none;',
        'border-top-image:',
        'border-right-image:',
        'border-bottom-image:',
        'border-left-image:',
　　　　'border-corner-image:',
        'border-top-left-image:',
        'border-top-right-image:',
        'border-bottom-left-image:',
        'border-bottom-right-image:',
        'border-radius:',
        'border-radius:none;',
        'border-top-left-radius:',
        'border-top-right-radius:',
        'border-bottom-right-radius:',
        'border-bottom-left-radius:',
        'box-shadow:',
        'box-shadow:1px 1px 1px #000;',
        //background
        'background-origin:border;',
        'background-origin:padding;',
        'background-origin:content;',
        'background-clip:border-box;',
        'background-clip:padding-box;',
        'background-clip:content-box;',
        'background-clip:no-clip;',
        'background-size:',
        'background-size:[length|percentage|auto]{1,2}|cover|contain;',
        'background-color:hsl();',
        'background-color:hsl(length||percentage||percentage);',
        'background-color:hsla();',
        'background-color:hsla(length||percentage||percentage||opacity);',
        'background-color:rgba();',
        'background-color:rgba(r||g||b||alpha);',
        'opacity:',
        //text effects
        'text-shadow:',
        'text-shadow:1px 1px 1px #000;',
        'text-overflow:clip;',
        'text-overflow:ellipsis;',
        'word-wrap:normal;',
        'word-wrap:break-word;',
        //user-interface
        'box-sizing:content-box;',
        'box-sizing:border-box;',
        'box-sizing:inherit;',
        'resize:none;',
        'resize:both;',
        'resize:horizontal;',
        'resize:vertical;',
        'resize:inherit;',
        //outline & outline-width & outline-style & outline-color see css2 part
        'outline-offset:',
        'nav-index:',
        'nav-index:auto;',
        'nav-up:',
        'nav-up:auto;',
        'nav-right:',
        'nav-right:auto;',
        'nav-down:',
        'nav-down:auto;',
        'nav-left:',
        'nav-left:auto;',
        'columns:',
        'column-width:',
        'column-span:',
        'column-rule:',
        'column-rule-color:',
        'column-rule-width:',
        'column-rule-style:',
        'column-gap:',
        'column-fill:',
        'column-count:',
        'column-break-before:',
        'column-break-after:'
    ],
    cssColor = [
        //http://www.w3school.com.cn/tags/html_ref_colornames.asp
        'aqua;',
        'black;',
        'blue;',
        'fuchsia;',
        'gray;',
        'green;',
        'lime;',
        'maroon;',
        'navy;',
        'olive;',
        'purple;',
        'red;',
        'silver;',
        'teal;',
        'white;',
        'yellow;'
    ],
    customCss = [
        'line-height:1.5;',
        'text-indent:-9999px;',
        'overflow:hidden;text-overflow:ellipsis;white-space:nowrap;',
        'background:url(images/) no-repeat;',
        'height:100%;',
        'width:100%;',
        'border:1px solid #000;',
        'margin:0 auto;',
        'zoom:1;',
        //pseudo-classes
        'link',
        'hover',
        'active',
        'visited',
        'first-child',
        'first',
        'focus',
        //pseudo-elements
        'after',
        'first-letter',
        'first-line',
        'before',
        'after',
        //declaration
        'important;',
        'filter:alpha(opacity=50);opacity:.5;',
        '"\\5B8B\\4F53"/*songti*/;',
        '"\\5FAE\\8F6F\96C5\9ED1"/*yahei*/,sans-serif/*for mac*/;',
        '"\\6977\\4F53"/*kaiti*/;',
        '"\\9ED1\\4F53"/*heti*/;',
        '-webkit-',
        'webkit-',
        'moz-',
        '-moz-',
        'o-',
        '-o-',
        'ms-',
        '-ms-',
    ]
    
	var finalArr = css2.concat(css3).concat(cssColor).concat(customCss);
	var tempArr = [], tempObj = {};
	//去重
	for(var i = 0, l = finalArr.length; i < l; i++){
		if(!tempObj[finalArr[i]]){
			tempObj[finalArr[i]] = 1;
			tempArr.push(finalArr[i]);
		}
	}
    
	//不区分大小写排序
	tempArr.sort(function(a,b){return compareStr(a.toLowerCase(),b.toLowerCase());});
    
	var rStr = '&lt;?xml version="1.0" encoding="Windows-1252" ?&gt;&lt;NotepadPlus&gt;&lt;AutoComplete&gt;';
	for(var i = 0, l = tempArr.length; i < l; i++){
		rStr += '<br/>'+getKeyItem(tempArr[i]);
	}
	rStr += '<br/>'+'&lt;/AutoComplete&gt;&lt;/NotepadPlus&gt;';
	$('#result').html(rStr);
	//get items
	function getKeyItem(p){
		return '&lt;KeyWord name="'+p.replace(/"/g,'&amp;quot;')+'" /&gt;';
	}
	function compareStr(a,b){
		if(a === b) return 0;
		var i = 0;
		while(true){
			if(a.charAt(i) && b.charAt(i)){
				if(a.charCodeAt(i) === b.charCodeAt(i)){
					i++;
				}else if(a.charAt(i) == ':'){
                    return -1;
                }else if(b.charAt(i) == ':'){
                    return 1;
                }else{
					return a.charCodeAt(i) > b.charCodeAt(i) ? 1 : -1;
				}
			}else{
				if(a.charAt(i)){
					return 1;
				}else{
					return -1;
				}
			}
		}
	}
}();